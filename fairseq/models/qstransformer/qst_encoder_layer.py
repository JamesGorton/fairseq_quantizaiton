...
from fairseq.models.transformer import TransformerConfig, qLinear
from fairseq.modules import LayerNorm, MultiheadAttention
from fairseq.modules.fairseq_dropout import FairseqDropout
from fairseq.modules.quant_noise import quant_noise


class QuantTransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return quant_noise(
            qLinear.qlinear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
        )
    ...
    def _prune_fc_layer(self, remove_index: List[int]):
        ...
        self.fc1 = quant_noise(
            qLinear.qlinear(self.fc1.in_features, self.fc1.out_features - len(remove_index)),
            p=self.quant_noise,
        ...
        self.fc2 = quant_noise(
            qLinear.qlinear(self.fc2.in_features - len(remove_index), self.fc2.out_features),
            p=self.quant_noise,
            block_size=self.quant_noise_block_size,
        )
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return quant_noise(qLinear.qlinear(input_dim, output_dim), q_noise, qn_block_size)
